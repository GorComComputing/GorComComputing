...
Центральной точкой притяжения, объединяющей компьютерные технологии в единое целое, является операционная система. Процесс её создания включает проектирование микрпроцессорной системы и описание структур и алгортмов ядра операционной системы. Вместе они создают пространтво для исполнения прикладных программ.
Каждая прикладная программа в такой системе видит свой плоский объем памяти, обладает квантом процессорного времени и для связи с внешними устройствами или другими программами использует вызовы ядра операционной системы.  
Внутри плоского объема памяти программа хранит свои стрктуры и алгоритмы. Если в прикладной программе разместить стрктуры и алгоритмы ядра операционнй системы, то получим операционную систему внутри операционной системы. Если опуститься глубже и дополнительно разместить внутри прикладной програмы струтры и алгоритмы, которые содержит микропроцессор, то мы получим компьютер внутри компьютера.
Таким образом, у нас есть возможность создавать неограниченное число компьютеров всех видов архитектур, как существующих, так и придумывать новые. Интересным выглядит тот методологический факт, что мы можем не просто запустить машину Тьюринга на современном Core i7, но и запусить Core i7 на машине Тьюринга. 
...
Сейчас я разрабатываю 3-ю версию, работающую на процессорах IA-32, ARM32, MIPS32 и RISC-V. Операционная система POSIX-совместима, что позволяет разрабатывать прикладные программы под Linux до того, как я завершу собственное ядро. Это означает, что программы для доступа к стрктурам и алгоритмам ядра используют общие для всех Unix-систем функции, а само ядро внутри может быть устроено на усмотрение программиста любым образом.
...
* [Операционная система Doors 3]() (на C и Assembly i386, ARM32, MIPS32, RISC-V) - `в разработке` :clock10:
